...
specific event which you can listen to as follows:
...
1. `getExpiryTime()` returns a `Time` instance representing the subscription
   expiry time.
2. `getItemId()` returns the subscription identifier.
3. `getProvider()` returns the subscription provider. (`app_store` or
`google_play`).
4. `getUniqueIdentifier()` returns the subscription unique identifier. Learn
more about customizing the subscription unique identifiers in the
[configuration](/docs/get-started/installation#unique-identifiers) section.
5. `getProviderRepresentation()` returns the subscription provider original
